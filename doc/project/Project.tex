...
Sadly the parallelization could not be fully implemented mainly due to performance problems, but this could have been an hardware defect from the SpiNNaker unit. It would however be interesting to investigate the parallel version on another hardware unit, as this approach should theoretically improve the performance.\\
In addition, other optimizations could be performed, especially during the slave updates sending. Indeed, the master could be a potential bottleneck of the system because it has to receive 64 updates (without counting all the camera events). A possible solution is to build relay populations (one for each chip), that gather information from their chip nodes, sum it and send to the master.\\ \\
