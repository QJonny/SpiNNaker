...
static vector2d n_pos; // noise
static vector2d n_sp; // noise


static float x_coef = 0.0;
static float y_coef = 0.0;
...
void move(uint sim_time) {
	...
	for(i = 0; i < N_A; i++) {
		if(i < N_A/4) { // x pos component
			p = &x_pos;
		}
		else if(i >= N_A/4 && i < N_A/2) { // y pos component
			p = &y_pos;
		}
		else if(i >= N_A/2 && (N_A*3)/4) { // x speed component
			p = &x_sp;
		}
		else { // y speed component
			p = &y_sp;
		}

		*p += w_A_array[i] * phi_A(i);
	}

	x_coef = (x_pos + x_sp) + pos_.x + speed_.x / fabs((x_pos + x_sp) + pos_.x + speed_.x);
	y_coef = (y_pos + y_sp) + pos_.y + speed_.y / fabs((y_pos + y_sp) + pos_.y + speed_.y);
	...
	// range: [-1;1]
	x_pos = -coef*(x_pos) + sigma() * n_pos.x;
	y_pos = coef*(y_pos) + sigma() * n_pos.y;
	x_sp = -coef*(x_sp) + sigma() * n_sp.x;
	y_sp = coef*(y_sp) + sigma() * n_sp.y;
	...
	x_ = range(x_pos + x_sp, 1.0, -1.0);
	y_ = range(y_pos + y_sp, 1.0, -1.0);
...
void update_A() { // TODO: has to change when it will be parallelized
	...
	for(i = 0; i < N_A; i++) {
		...
		//w_A_array[i] += LEARNING_RATE_A*error*n*delta_A_wi(i) / N_A;
		w_A_array[i] += LEARNING_RATE_A*delta_A_wi(i)*c;
	}
