...
static float w_A_array[N_A] = {0.01};
...
void updateError(int x_pos, int y_pos, int x_speed, int y_speed) {
	speed_ = normalizeBallParams(x_pos, y_pos, x_speed, y_speed);
	float r = R(speed_);
	float curr_V = V();
...
void move(int time) {
	...
	sendMotorCommand((int)(x_*1000) + 2000, (int)(y_*1000) + 2000);
}


void update_A(int time) { // TODO: has to change when it will be parallelized
	int i = 0;
	n = vector(1.0, 1.0);//noise(time);
