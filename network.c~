...
static float w_V_array[N_V] = {0.5};
static float phi_V_old[N_V] = {0.0};

static float w_A_theta_array[N_A] = {0.5};
static float w_A_phi_array[N_A] = {0.5};
...
float phi_V(int index){
	return mfm_(index);//(2.0 - v_norm(v_sub(pos_, center[index])))/2.0;
}
